...
Haciendo un breve resumen hemos encontrado un patron que se repite a lo largo de todas las semanas, durante los lunes, martes y miércoles tenemos los registros más bajos en cunto a decibelios, pero a medida que se acerca el jueves y el fin de semana los nivels registrados por los sonómetros se disparan.
  ...
  *Respuesta*
  ...
  Una vez con el gráfico hecho y tras analizar en más profundidad las festividades que cumplen los requisitos en cuanto a duración y calibre (gente a la que atrae la festividad, nivel de actividad en las calles debido a la fiesta en cuestiónb) de la festividad concluimos que vemos cambios significativos en el mes de marzo, cosa que esperabamos ya que en este mes se celebra una de las fiestas más importantes de Valencia, sino la más importante. Notamos un amuento en los niveles registrados ya que en estas fiestas las actividades principales són hacer mascletaes, fuegos artificiales y de más actividades que generen un alto nivel sonoro. 
...
Creamos una función para detectar Outliers, existen distintos métodos, en este caso vamos a utilizar el método 3 sigma.

```{r}
#Esta funcíón detecta los outliers y crea una lista con valores binarios para indicarlo.
VecTresSigma <- function(x, na.rm=T){
  media = mean(x, na.rm=na.rm)
  sigma = sd(x, na.rm=na.rm)
  LimInf = media - 3*sigma
  LimSup = media + 3*sigma
...
TresSigma <- function(x, na.rm = T){
  media = mean(x, na.rm = na.rm)
  sigma = sd(x, na.rm = na.rm)
  
  LimInf = media - 3*sigma
  LimSup = media + 3*sigma
  n_dentro = length(which(x > LimInf | x < LimSup))
  ...
  perc_dentro = n_dentro/length(x)
  
  vec_info = cbind(media, n_dentro, LimInf, LimSup, max_fuera, max_dentro, min_fuera, min_dentro, perc_dentro)
  rownames(vec_info) = NULL
  
  return (vec_info)
}
```

Realizamos el rastreo de outlier con respecto a las distintas calles y los representamos. Para ello hacemos uso de la primera función:
...
for (calle in l_calles){
  ...
  result <- VecTresSigma(datos_calle$valor)
...
ggplot(outliers_calle, aes(calle,número_outliers)) +  
  geom_point() + 
  theme(axis.text.x = element_text(angle = 20, size = 6,hjust = 1, vjust = 1)) +
  labs(title = "Outliers", y = "", x = "Calle", subtitle = "Número de outliers en función de la calle según la regla del 3 sigma")

```

También podemos representar todos los datos en función de su calle, representando quales son los límites superiores he inferiores que estipula el método de detección. Para ello hemos de utilizar la segunda función.

```{r}
sol_sigma <- TresSigma(df$valor)
...
ggplot(df, aes(Nombre_calle, valor, fill = Nombre_calle)) +  
  ...
  geom_hline(yintercept = sol_sigma[3], col = "red") + 
  geom_hline(yintercept = sol_sigma[4], col = "blue") + 
  geom_hline(yintercept = sol_sigma[1], col = "green", linetype = 2)  +
  theme_bw() +
...
Si nos damos cuenta nuestra hipostesis de que en las festividades más grandes debia haber un nivel de ruido mayor, realmente solo se cumple en el mes de marzo con la festividad de las fallas en éste. Pero obviamente un solo caso no indica nada y más conociendo la festividad de las fallas. Festividad donde las actividades principales son el lanzamiento de petardos, tracas, castillos, mascletaes y fuegos artificiales entre otros. Todos estos modifican de manera considerable el nivel sonoro del barrio en el mes de marzo.
...
total %>% filter(tipo_medida == "Nivel_sonoro_dia") %>% ggplot(aes(x = valor))+
  geom_histogram()+
  geom_vline(xintercept = 55, col = 2) +
  geom_vline(xintercept = 65, col = 2) +
  geom_text(aes(55,-10,label = "Residencial")) +
  geom_text(aes(65,-10,label = "Terciario")) +
...
total %>% filter(tipo_medida == "Nivel_sonoro_noche") %>% ggplot(aes(x = valor))+
  ...
  geom_vline(xintercept = 55, col = 2) +
  geom_text(aes(45,-10,label = "Residencial")) +
  geom_text(aes(55,-10,label = "Terciario")) +
